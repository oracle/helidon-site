...
<dd slot="desc"><p>This tutorial describes how to build an Helidon MicroProfile (MP) application from scratch
...
&lt;project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    &lt;artifactId&gt;helidon-mp-tutorial&lt;/artifactId&gt;  <span class="conum" data-value="1" />
    &lt;version&gt;1.0-SNAPSHOT&lt;/version&gt;
    &lt;packaging&gt;jar&lt;/packaging&gt;
    &lt;name&gt;${project.artifactId}&lt;/name&gt;

    &lt;properties&gt;
        &lt;helidon.version&gt;1.3.0&lt;/helidon.version&gt;
        &lt;mainClass&gt;io.helidon.examples.Main&lt;/mainClass&gt;
        &lt;maven.compiler.source&gt;8&lt;/maven.compiler.source&gt; <span class="conum" data-value="2" />
    ...
    &lt;dependencyManagement&gt;
        &lt;dependencies&gt;
            &lt;dependency&gt;
                ...
                &lt;artifactId&gt;helidon-bom&lt;/artifactId&gt; <span class="conum" data-value="3" />
    ...
    &lt;dependencies&gt;
        &lt;dependency&gt;
            ...
            &lt;artifactId&gt;helidon-microprofile-2.2&lt;/artifactId&gt; <span class="conum" data-value="4" />
        &lt;/dependency&gt;
    &lt;/dependencies&gt;
...
<ul class="colist">
<li data-value="1">Establishes the Maven coordinates for the new project.</li>
<li data-value="2">Tells Maven to use Java 8 for the source
and target versions as defined in the <code>properties</code> section, and
sets the <code>mainClass</code> which will be used later
when building a JAR file.  The class will be created later in this
tutorial.</li>
<li data-value="3">Include the Helidon BOM POM in the <code>dependencyManagement</code> section.
...
module without worrying about the version it should depend on.</li>
<li data-value="4">Adds a dependency for the MicroProfile 2.2 bundle which allows the use of
MicroProfile 2.2 features in the application.</li>
</ul>
<div class="admonition tip">
<p class="admonition-inline">MicroProfile 2.2 contains features like Metrics, Health Check,
 ...
 <a id="" title="" target="_blank" href="https://projects.eclipse.org/projects/technology.microprofile/releases/microprofile-2.2">Eclipse MicroProfile 2.2</a> site.</p>
</div>
...
<div class="admonition tip">
<p class="admonition-inline">Technically, the main class is not needed unless you want to control
 the startup sequence.  You can set the <code>mainClass</code> to
 <code>io.helidon.microprofile.Main</code> and it will find the application
 annotated with <code>ApplicationScoped</code>.  Though it is likely that
 almost any "real" application would want its own main class.</p>
</div>
<p>Create a new Java class inside that directory called <code>GreetApplication.java</code>
 and add the following content:</p>

<markup
lang="java"
title="Initial application class"
...
public class GreetApplication extends Application { <span class="conum" data-value="3" />
    ...
    @Override
    public Set&lt;Class&lt;?&gt;&gt; getClasses() { <span class="conum" data-value="4" />
...
<ul class="colist">
<li data-value="1">The class is annotated with <code>ApplicationScoped</code>
...
application.</li>
<li data-value="2">The <code>ApplicationPath</code> annotation sets the URL path that will be used to
access the application.</li>
<li data-value="3">The application class must extend <code>javax.ws.rs.core.Application</code>.</li>
<li data-value="4">In the application class, override the
<code>public Set&lt;Class&lt;?&gt;&gt; getClasses()</code> method.  This method must return
a <code>Set</code> of <code>Class</code> objects, one for each resource that the application
will serve.  Currently, the application only has one resource, the
<code>GreetResource</code>.</li>
</ul>
<div class="admonition tip">
<p class="admonition-inline">You can learn more about scopes and contexts, and how they are used
 from the <a id="" title="" target="_blank" href="https://docs.jboss.org/cdi/api/2.0/index.html">Specification</a>.</p>
</div>
...
<p>A main class is also required to start up the server and run the
 application.  Here is the initial main class:</p>

<markup
lang="java"
title="Initial Main class"
>package io.helidon.examples;
...
<h2 id="_update_the_pom_to_build_the_application">Update the POM to build the application</h2>
<div class="section">
<p>Helidon MP applications are packaged into a JAR file and the dependencies
 are copied into a <code>libs</code> directory.  A detailed explanation of
 why this particular packaging approach is recommended is provided on the
 <router-link to="#guides/30_dockerfile" @click.native="this.scrollFix('#guides/30_dockerfile')">Creating Docker Images</router-link> page.
 Update the Maven POM file to create the correct packaging.  Here is
 the updated file:</p>

<markup
lang="xml"
title="Updated POM file"
>&lt;?xml version="1.0" encoding="UTF-8"?&gt;
...
&lt;project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    &lt;properties&gt;
        ...
        &lt;maven.compiler.target&gt;${maven.compiler.source}&lt;/maven.compiler.target&gt;
        &lt;libs.classpath.prefix&gt;libs&lt;/libs.classpath.prefix&gt; <span class="conum" data-value="1" />
    ...
    &lt;build&gt;
        &lt;pluginManagement&gt;
            &lt;plugins&gt;
                &lt;plugin&gt;
                    &lt;groupId&gt;org.apache.maven.plugins&lt;/groupId&gt; <span class="conum" data-value="2" />
        ...
        &lt;plugins&gt;
            &lt;plugin&gt;
                ...
                &lt;artifactId&gt;maven-dependency-plugin&lt;/artifactId&gt; <span class="conum" data-value="3" />
                &lt;executions&gt;
                    &lt;execution&gt;
...
<ul class="colist">
<li data-value="1">New entries are created in the <code>properties</code> to set the name of the <code>libs</code>
directory where the dependencies will be placed.</li>
<li data-value="2">In the <code>build</code> section,
a <code>plugin</code> entry is added to configure the <code>maven-jar-plugin</code> to set the
main class in the manifest, and to add the <code>libs</code> directory to the
manifest classpath.</li>
<li data-value="3">Additionally, the <code>maven-dependency-plugin</code> is
configured to copy the dependencies into the <code>libs</code> directory.</li>
</ul>
<p>With these updates in place, build the application with Maven:</p>
...
>java -jar target/helidon-mp-tutorial-1.0-SNAPSHOT.jar</markup>
...
<p class="admonition-inline">You can learn more about options for configuring the Helidon Server on the
 <router-link to="#miroprofile/02_server-configuration" @click.native="this.scrollFix('#miroprofile/02_server-configuration')">Configuring the Server</router-link> page.</p>
</div>
...
<p class="admonition-inline">To learn more about Helidon MP configuration please see the
 <router-link to="/config/01_introduction">Config</router-link> section of the documentation.</p>
</div>
</div>
...
handlers=java.util.logging.ConsoleHandler <span class="conum" data-value="1" />

# Global default logging level. Can be overriden by specific handlers and loggers
.level=INFO <span class="conum" data-value="2" />

# Helidon Web Server has a custom log formatter that extends SimpleFormatter.
# It replaces "!thread!" with the current thread name
java.util.logging.ConsoleHandler.level=INFO
java.util.logging.ConsoleHandler.formatter=io.helidon.webserver.WebServerLogFormatter <span class="conum" data-value="3" />
java.util.logging.SimpleFormatter.format=%1$tY.%1$tm.%1$td %1$tH:%1$tM:%1$tS %4$s %3$s !thread!: %5$s%6$s%n <span class="conum" data-value="4" /></markup>

<ul class="colist">
<li data-value="1">The console logging handler is configured.</li>
<li data-value="2">The default logging level is set to <code>INFO</code>.</li>
<li data-value="3">The Helidon custom log formatter is used, this formatter extends the
<code>SimpleFormatter</code>.</li>
<li data-value="4">The format string is set using the standard options to include the timestamp,
thread name and message.</li>
</ul>
<div class="admonition tip">
<p class="admonition-inline">Remember that when you set the default level to <code>INFO</code>, if you set an
 individual logger to a higher level (like <code>FINER</code>) you will still only get
 <code>INFO</code> level messages.  A common practice is to set the default level higher
 and set the individual loggers to <code>INFO</code>.</p>
</div>
<p>Update the main class to configure logging as shown below:</p>

<markup
lang="java"
title="Updated main class with logging"
>import java.util.logging.LogManager;

    // some lines omitted

    public static void main(final String[] args) throws IOException {
        setupLogging(); <span class="conum" data-value="1" />
        Server server = startServer();
        System.out.println("http://localhost:" + server.port() + "/greet");
    }

    private static void setupLogging() throws IOException { <span class="conum" data-value="2" />
        LogManager.getLogManager().readConfiguration(
                Main.class.getResourceAsStream("/logging.properties"));
    }</markup>

<ul class="colist">
<li data-value="1">In the <code>main()</code> method introduce a call to a new <code>setupLogging()</code> method.</li>
<li data-value="2">This new method reads the logging configuration from the <code>logging.properties</code>
file and updates the <code>LogManager</code> to use that configuration.</li>
</ul>
...
<p>Learn more about logging in the <router-link to="#guides/xx_logging.adoc" @click.native="this.scrollFix('#guides/xx_logging.adoc')">Logging Guide</router-link>.</p>
...
<p>Learn more about using Helidon and MicroProfile metrics in the <router-link to="/guides/09_metrics_mp_guide">Metrics Guide</router-link>.</p>
...
<markup
lang="java"
title="New GreetHealthcheck.java custom health check class"
>package io.helidon.examples;
...
<p>Learn more about health checks in the <router-link to="#xx_health-checks.adoc" @click.native="this.scrollFix('#xx_health-checks.adoc')">Health Check
Guide</router-link>.</p>
...
>FROM maven:3.5.4-jdk-9 as build <span class="conum" data-value="1" />
WORKDIR /helidon
...
FROM openjdk:8-jre-slim <span class="conum" data-value="4" />
WORKDIR /helidon

COPY --from=build /helidon/target/helidon-mp-tutorial-1.0-SNAPSHOT.jar ./ <span class="conum" data-value="5" />
COPY --from=build /helidon/target/libs ./libs

CMD ["java", "-jar", "helidon-mp-tutorial-1.0-SNAPSHOT.jar"] <span class="conum" data-value="6" />
EXPOSE 8080</markup>
...
<p>Learn more about creating Docker images for Helidon applications at
 <router-link to="/guides/30_dockerfile">Creating Docker Images</router-link>.</p>
...
<ul class="ulist">
<li>
<p><a id="" title="" target="_blank" href="https://projects.eclipse.org/projects/technology.microprofile/releases/microprofile-2.2">Eclipse MicroProfile 2.2</a></p>
...
<li>
<p><router-link to="#guides/04_dockerfile" @click.native="this.scrollFix('#guides/04_dockerfile')">Creating Docker Images</router-link></p>

</li>
<li>
<p><router-link to="#miroprofile/02_server-configuration" @click.native="this.scrollFix('#miroprofile/02_server-configuration')">Configuring the Server</router-link></p>

</li>
<li>
<p><router-link to="/config/01_introduction">Config</router-link></p>

</li>
<li>
<p><router-link to="#guides/xx_logging.adoc" @click.native="this.scrollFix('#guides/xx_logging.adoc')">Logging Guide</router-link></p>
...
<li>
<p><router-link to="/guides/09_metrics_mp_guide">Metrics Guide</router-link></p>
...
<li>
<p><router-link to="#getting-started/05_kubernetes.adoc" @click.native="this.scrollFix('#getting-started/05_kubernetes.adoc')">Install Kubernetes on your desktop</router-link></p>
