...
<p>MicroProfile Config supports a concept of configuration profiles. You can define a profile using the configuration property <code>mp.config.profile</code> (when using default configuration, this can be defined as a system property, environment variable or as a property in <code>microprofile-config.properties</code>). When a profile is defined, additional config source is loaded (<code>microprofile-config-profile.properties) and properties from profile have precedence over default properties. Profile properties can be defined using `%profile</code> prefix, such as <code>%dev.server.port</code>.</p>
