...
<dd slot="desc"><p>The gRPC Microprofile APIs are an extension to <router-link to="/microprofile/01_introduction">Helidon MP</router-link> to allow building
...
The important parts in the example are the <code>@ApplicationScoped</code>, <code>@RpcService</code> and <code>@Unary</code> annotations; these,
...
@io.helidon.microprofile.grpc.core.RpcService     <span class="conum" data-value="2" />
...
<li data-value="2">The <code>RpcService</code> annotation is what defines the class as a gRPC service so that when the bean is iscovered it is
then deployed by the gRPC MP server.</li>
</ul>

<h3 id="_service_name">Service Name</h3>
<div class="section">
<p>By default when a class is annotated with <code>RpcService</code> the class name will be used as the gRPC service name. So in the example
...
>@ApplicationScoped
@io.helidon.microprofile.grpc.core.RpcService(name="Strings") <span class="conum" data-value="1" />
public class StringService {</markup>
...
<router-link to="/grpc/04_service_implementation">non-microprofile Helidon gRCP services</router-link>.</p>
...
the <router-link to="/grpc/03_routing">basic gRPC server documentation</router-link>).</li>
</ul>
