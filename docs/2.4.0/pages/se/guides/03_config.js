...
"app-greeting".  See <router-link to="/se/config/06_advanced-configuration">Advanced Config</router-link> for more information.</p>
</div>
...
See <router-link to="/se/config/06_advanced-configuration">Advanced Config</router-link> for more information.</p>
...
<h3 id="_configuration_profiles">Configuration Profiles</h3>
<div class="section">
<p>Instead of directly specifying the configuration sources in your code, you can use a profile file that declares
the configuration sources and their attributes.</p>

<p>Simplest way to use a profile is to define a <code>config-profile.yaml</code> (and possible other files, such as
<code>config-profile-dev.yaml</code> for <code>dev</code> profile) on classpath or on file system, and create config using <code>Config.create()</code>.
The profile can be changed by a system property <code>config.profile</code>, or using an environment variable <code>HELIDON_CONFIG_PROFILE</code>.</p>

<p>Profile file can use any supported format, following example is using <code>YAML</code>.</p>

<markup
lang="yaml"
title="Create a file named <code>cofnig-profile.yaml</code> in the <code>helidon-quickstart-se</code> directory with the following contents:"
>sources:
...
<ul class="colist">
<li data-value="1">Will use <code>config-profile.yaml</code> by default</li>
</ul>
<markup
lang="bash"
title="Build and run the application, then invoke the endpoint and check the response:"
...
<p>The source precedence order in a profile file is the order of appearance in the file.
...
<p>When using a profile file, you need to explicitly include both environment variables and system properties as
...
methods as described in <router-link to="/se/config/03_hierarchical-features">Hierarchical Config</router-link> and <router-link to="/se/config/06_advanced-configuration">Advanced Config</router-link>.</p>
...
See <router-link to="/se/config/05_mutability-support">Config Mutability Support</router-link> for a full discussion on this topic.
...
introduction section at <router-link to="/se/config/01_introduction">Helidon Config</router-link> has much more information including
the following:</p>
