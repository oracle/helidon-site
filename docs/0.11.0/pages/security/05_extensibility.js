...
<div class="section">
<p>Security provides integration with Helidon WebServer (<code>helidon-security-integration-webserver</code>)
 and with Jersey (<code>helidon-security-integration-jersey</code>).</p>

<p>These integrations use APIs that are available to integrate any framework/application (for example
we could integrate security with messaging, such as JMS).</p>
...
<p><code>Security</code> is then used to create an instance of <code>SecurityContext</code>, that should be used
for interaction with a single user - in HTTP, we create a single <code>SecurityContext</code> per
request.</p>

<p><code>SecurityContext</code> is used to invoke authentication, authorization, and outbound security requests.</p>

<p>Helidon security also defines a set of annotations:
...
be annotated, the integration should at least support the annotations mentioned above.</p>
